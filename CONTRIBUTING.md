...
- One spec file per class/module. For each class or module under `lib/`, keep all of its unit tests in a single spec file under `spec/` that mirrors the path and file name exactly: `lib/month/serializer/my_class.rb` -> `spec/month/serializer/my_class_spec.rb`.
...
[🧪build]: https://github.com/galtzo-floss/month-serializer/actions
